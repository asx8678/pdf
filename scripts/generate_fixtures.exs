...
defmodule GenerateFixtures do
  ...
  defp assemble(objs) do
    ...
    {offs, _} =
      Enum.reduce(objs, {[], byte_size(hdr)}, fn {_, t}, {a, p} -> {[p | a], p + byte_size(t)} end)
  ...
  # Hand-assembled xref tables are rejected by the lopdf NIF parser (T-115).
  # The scanned fixture feeds the T-125 accept path (`Quire.Pdf.open`), so it
  # must be re-serialised through PDFium at generation time.
  defp normalize(bytes) do
    {:ok, doc} = ExPdfium.open_blob(bytes)
    {:ok, out} = ExPdfium.save_to_bytes(doc)
    ExPdfium.close(doc)
    out
  end
  ...
  defp pdfs do
    ...
    # A synthetic scanned form: closed boxes, a checkbox, and two underline
    # blanks — drawn as vector graphics (no text), so extract_text still
    # reports nothing while the line/box detector finds real fields (T-125).
    scanned_ct =
      "1 w\n" <>
        "0 0 0 RG\n" <>
        "72 700 178 20 re S\n" <>
   ...
   /Contents 5 0 R >>\nendobj\n"},
    ...
    # xfa_form.pdf
    xfa =
      ~s|<?xml version="1.0"?><xdp:xdp xmlns:xdp="http://ns.adobe.com/xdp/"><form1><TextField1>Hello</TextField1></form1></xdp:xdp>|
    ...
    spdf("encrypted_user_pw.pdf", "Encrypted", [{6, enc}], "/Encrypt 6 0 R ")
    IO.puts("  encrypted_user_pw.pdf")
    spdf("encrypted_owner_pw.pdf", "Encrypted", [{6, enc}], "/Encrypt 6 0 R ")
    IO.puts("  encrypted_owner_pw.pdf")
    ...
    spdf(
      ...
      "/StructTreeRoot 6 0 R /MarkInfo << /Marked true >> /Lang (en-US) "
    )

    IO.puts("  tagged_accessible.pdf")

    # rotated_pages.pdf — 4 pages at 0, 90, 180, 270
    rots = [0, 90, 180, 270]

    rpages =
      Enum.with_index(rots, fn r, i ->
        cn = 6 + i * 2
        pn = 7 + i * 2
    ...
    mpages =
      Enum.with_index(sizes, fn {l, w, h}, i ->
        cn = 6 + i * 2
        pn = 7 + i * 2
    ...
    mpage_ids =
      Enum.map_join(sizes |> Enum.with_index(), " ", fn {{_, _, _}, i} -> "#{7 + i * 2} 0 R" end)
    ...
    {:ok, {_, docx}} =
      :zip.create(
        ...
        [
          ...
          {~c"_rels/.rels",
           ~S|<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/></Relationships>|},
          {~c"word/document.xml",
           ~S|<?xml version="1.0"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>Hello World - Report</w:t></w:r></w:p></w:body></w:document>|}
        ],
        [:memory]
      )
    ...
    {:ok, {_, xlsx}} =
      :zip.create(
        ...
        [
          ...
          {~c"xl/workbook.xml",
           ~S|<?xml version="1.0"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheets><sheet name="Sheet1" sheetId="1" r:id="rId1" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"/></sheets></workbook>|},
          ...
          {~c"xl/worksheets/sheet1.xml",
           ~S|<?xml version="1.0"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData><row r="1"><c r="A1" t="inlineStr"><is><t>Item</t></is></c><c r="B1" t="inlineStr"><is><t>Amount</t></is></c></row><row r="2"><c r="A2" t="inlineStr"><is><t>Revenue</t></is></c><c r="B2" t="inlineStr"><is><t>$1000</t></is></c></row></sheetData></worksheet>|}
        ],
        [:memory]
      )
    ...
    {:ok, {_, pptx}} =
      :zip.create(
        ...
        [
          ...
          {~c"_rels/.rels",
           ~S|<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="ppt/presentation.xml"/></Relationships>|},
          {~c"ppt/presentation.xml",
           ~S|<?xml version="1.0"?><p:presentation xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:sldIdLst><p:sldId id="256" r:id="rId1" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"/></p:sldIdLst><p:sldSz cx="9144000" cy="6858000"/></p:presentation>|},
          {~c"ppt/_rels/presentation.xml.rels",
           ~S|<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide" Target="slides/slide1.xml"/></Relationships>|},
          {~c"ppt/slides/slide1.xml",
           ~S|<?xml version="1.0"?><p:sld xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:cSld><p:spTree><p:nvGrpSpPr><p:cNvPr id="1" name=""/></p:nvGrpSpPr><p:grpSpPr/><p:sp><p:nvSpPr><p:cNvPr id="2" name="Title 1"/></p:nvSpPr><p:spPr/><p:txBody><a:bodyPr/><a:lstStyle/><a:p><a:r><a:rPr lang="en-US"/><a:t>Slide 1</a:t></a:r></a:p></p:txBody></p:sp></p:spTree></p:cSld></p:sld>|}
        ],
        [:memory]
      )
    ...
    {:ok, {_, odt}} =
      :zip.create(
        ...
        [
          ...
          {~c"content.xml",
           ~S|<?xml version="1.0"?><office:document xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0" xmlns:text="urn:oasis:names:tc:opendocument:xmlns:text:1.0"><office:body><office:text><text:p>Notes</text:p></office:text></office:body></office:document>|},
          {~c"META-INF/manifest.xml",
           ~S|<?xml version="1.0"?><manifest:manifest xmlns:manifest="urn:oasis:names:tc:opendocument:xmlns:manifest:1.0" manifest:version="1.2"><manifest:file-entry manifest:full-path="/" manifest:version="1.2" manifest:media-type="application/vnd.oasis.opendocument.text"/><manifest:file-entry manifest:full-path="content.xml" manifest:media-type="text/xml"/></manifest:manifest>|}
        ],
        [:memory]
      )
  ...
  defp images do
    ...
    write(
      ...
      <<137, 80, 78, 71, 13, 10, 26, 10, 0, 0, 0, 13, 73, 72, 68, 82, 0, 0, 0, 1, 0, 0, 0, 1, 8,
        2, 0, 0, 0, 144, 119, 83, 222, 0, 0, 0, 17, 73, 68, 65, 84, 120, 156, 99, 248, 207, 192,
        0, 0, 0, 255, 0, 1, 8, 0, 0, 20, 0, 0, 0, 0, 128, 0, 0, 0, 0, 0, 73, 69, 78, 68, 174, 66,
        96, 130>>
    )
    ...
    png2 =
      <<137, 80, 78, 71, 13, 10, 26, 10, 0, 0, 0, 13, 73, 72, 68, 82, 0, 0, 0, 2, 0, 0, 0, 1, 8,
        2, 0, 0, 0, 181, 168, 131, 24, idat::binary, iend::binary>>
    ...
    vips_fixtures = fn ->
      ...
      write(@img, "screenshot.png", png)
      IO.puts("  screenshot.png")
      ...
      write(@img, "scan_gray.tiff", tiff)
      IO.puts("  scan_gray.tiff")
      ...
      write(@img, "transparent.png", t_png)
      IO.puts("  transparent.png")
    ...
    ihdr_huge =
      <<0, 0, 0, 13, 73, 72, 68, 82, 0, 1, 134, 160, 0, 0, 0, 1, 8, 2, 0, 0, 0>> <>
        <<c_huge.("IHDR" <> <<0, 1, 134, 160, 0, 0, 0, 1, 8, 2, 0, 0, 0>>)::32>>

    l_huge = byte_size(comp_huge)

    idat_huge =
      <<l_huge::32, 73, 68, 65, 84, comp_huge::binary, c_huge.("IDAT" <> comp_huge)::32>>
    ...
    write(
      @img,
      "huge_dimensions.png",
      <<137, 80, 78, 71, 13, 10, 26, 10, ihdr_huge::binary, idat_huge::binary, iend_huge::binary>>
    )
    ...
    truncate_fixtures = fn ->
      ...
      write(@img, "truncated.jpg", truncated)
      IO.puts("  truncated.jpg")
      ...
      exif_ifd =
        <<0x49, 0x49, 0x2A, 0x00, 8, 0, 0, 0, 1, 0, 0x12, 0x01, 3, 0, 1, 0, 0, 0, 6, 0, 0, 0, 0,
          0, 0, 0>>

      app1 =
        <<0xFF, 0xE1, 2 + byte_size(exif_ifd)::16, 0x45, 0x78, 0x69, 0x66, 0x00, 0x00,
          exif_ifd::binary>>

      <<0xFF, 0xD8, rest::binary>> = exif_raw
      write(@img, "exif_rotated.jpg", <<0xFF, 0xD8, app1::binary, rest::binary>>)
      IO.puts("  exif_rotated.jpg")
    end
    ...
    if vips do
      try do
        ...
        write(@img, "multipage.tiff", multi)
        IO.puts("  multipage.tiff")
      rescue
