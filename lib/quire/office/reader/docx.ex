defmodule Quire.Office.Reader.Docx do
  ...
  @doc false
  def extract_image(state, rid, alt) do
    ...
    if target == nil do
      ...
      case Map.fetch(state.entry_map, path) do
        ...
        :error ->
          note = %{level: :unsupported, message: "Image not found: #{path}", source: "docx"}
          {%{state | notes: [note | state.notes], image_rid: nil, image_alt: nil}, nil}
      end
    end
  end
  ...
  def match_ns(name, local) do
    name == local or
      name == "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}#{local}"
  end
  ...
  defp parse_styles(entry_map) do
    case Map.fetch(entry_map, "word/styles.xml") do
      {:ok, xml} ->
        {:ok, result} = Saxy.parse_string(xml, StylesHandler, %{})
        result
  ...
  defp parse_rels(entry_map) do
    case Map.fetch(entry_map, "word/_rels/document.xml.rels") do
      {:ok, xml} ->
        {:ok, result} = Saxy.parse_string(xml, RelsHandler, %{})
        result
  ...
  defp do_group_lists([{:list_item, item, ordered} | rest], list_acc, result),
    do: do_group_lists(rest, [{item, ordered} | list_acc], result)

  defp do_group_lists([block | rest], [], result),
    do: do_group_lists(rest, [], [block | result])

  defp do_group_lists([block | rest], list_acc, result) do
    items = Enum.map(Enum.reverse(list_acc), fn {i, _} -> i end)
    ordered = elem(List.last(list_acc), 1)
    do_group_lists([block | rest], [], [{:list, items, ordered} | result])
  end

  defp do_group_lists([], [], result), do: Enum.reverse(result)

  defp do_group_lists([], list_acc, result) do
    items = Enum.map(Enum.reverse(list_acc), fn {i, _} -> i end)
    ordered = elem(List.last(list_acc), 1)
    Enum.reverse([{:list, items, ordered} | result])
  end
  ...
  defp extract_title(entry_map) do
    case Map.fetch(entry_map, "docProps/core.xml") do
      {:ok, xml} ->
        {:ok, title} = Saxy.parse_string(xml, CorePropsHandler, nil)
        title
  ...
  defmodule DocumentHandler do
    ...
    defp extract_img(st, r, a), do: Quire.Office.Reader.Docx.extract_image(st, r, a)
  ...
  # ═════════════════════════════════════════════════════════════════════════════
  # StylesHandler — builds %{lowered_style_id => style_name}
  # ═════════════════════════════════════════════════════════════════════════════
  ...
  defmodule StylesHandler do
    ...
    def handle_event(:characters, chars, {:style_name, id, type, _, acc}),
      do: {:ok, {:style_name, id, type, chars, acc}}

    def handle_event(:characters, chars, {:style, id, type, buf, acc}),
      do: {:ok, {:style, id, type, (buf || "") <> chars, acc}}
